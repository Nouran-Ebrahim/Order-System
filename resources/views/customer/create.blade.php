...
@section('content')
    <form class="col-8 mx-auto" action="{{ route('customers.store') }}" method="POST">
        @csrf
        @method('post')
        ...
        <div class="input-group mb-3">
            ...
            <input value="{{ old('country') }}" type="text" name="country" class="form-control" placeholder="Country"
                aria-label="Country">
            @error('country')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
